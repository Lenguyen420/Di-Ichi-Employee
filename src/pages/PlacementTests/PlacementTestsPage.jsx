...
export const PlacementTestsPage = () => {
  ...
  const saveResultStatus = () => {
    ...
    toast.success(t('Đã cập nhật trạng thái kết quả test.'))
  }

  return (
    <div className="space-y-5">
      <div>
        <p className="text-sm font-bold text-orange-600">{t('Kiểm tra đầu vào')}</p>
        <h1 className="mt-1 text-2xl font-black text-slate-950 md:text-3xl">{t('Danh sách kết quả test')}</h1>
        <p className="mt-2 text-sm text-slate-500">{t('Lưu và theo dõi kết quả kiểm tra đầu vào, trình độ, gợi ý khóa học và trạng thái tư vấn.')}</p>
      </div>

      <Card className="rounded-lg">
        <div className="grid gap-3 lg:grid-cols-[minmax(260px,1fr)_180px_180px]">
          <FilterField label={t('Lọc theo tên học sinh')}>
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <input
                className="!pl-10"
                placeholder={t('Nhập tên học sinh cần tìm')}
                value={searchName}
                onChange={(event) => setSearchName(event.target.value)}
              />
            </div>
          </FilterField>
          <FilterField label={t('Từ ngày')}>
...
const EditStatusModal = ({ result, status, statuses, onChange, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/20 p-4">
      <button className="absolute inset-0 cursor-default" type="button" onClick={onClose} aria-label={t('Đóng sửa trạng thái')} />
      <section className="relative z-10 w-full max-w-md rounded-lg border border-orange-100 bg-white p-5 shadow-2xl shadow-slate-950/20">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-bold text-orange-600">{t('Kết quả test')}</p>
        ...
        <label className="mt-5 block">
          <span className="text-sm font-black text-slate-700">{t('Trạng thái')}</span>
          <select
        ...
        <div className="mt-5 flex justify-end gap-3">
          <Button variant="ghost" type="button" onClick={onClose}>{t('Hủy')}</Button>
          <Button type="button" onClick={onSave}>{t('Lưu trạng thái')}</Button>
        </div>
      </section>
    </div>
  )
