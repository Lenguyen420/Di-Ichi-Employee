...
export const PotentialCandidatesTable = ({ candidates, onDelete, onEdit, onView, rowNumberOffset = 0 }) => {
  ...
  const columns = [
    ...
    { header: 'Giới tính', cell: ({ row }) => t(row.original.gender) },
    ...
    { header: 'Trạng thái', cell: ({ row }) => <Badge tone={candidateStatusTone[row.original.status]}>{t(row.original.status)}</Badge> },
    {
      header: 'Thao tác',
      cell: ({ row }) => (
        <div className="flex min-w-28 items-center gap-1">
          <button className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 transition hover:bg-orange-50 hover:text-orange-700" type="button" onClick={() => onView(row.original)} aria-label={t('Xem ứng viên')}>
            <Eye size={16} />
          </button>
          <button className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 transition hover:bg-orange-50 hover:text-orange-700" type="button" onClick={() => onEdit(row.original)} aria-label={t('Sửa ứng viên')}>
            <Pencil size={16} />
          </button>
          <button className="grid h-8 w-8 place-items-center rounded-lg text-rose-500 transition hover:bg-rose-50 hover:text-rose-700" type="button" onClick={() => onDelete(row.original)} aria-label={t('Xóa ứng viên')}>
            <Trash2 size={16} />
          </button>
        </div>
      ),
