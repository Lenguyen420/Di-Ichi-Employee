...
export const PotentialCandidatesHeader = ({ onExport, onImportFile, onToggleForm }) => {
  ...
  return (
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
      <div>
        <p className="text-sm font-bold text-orange-600">{t('Ứng viên tiềm năng')}</p>
        <h1 className="mt-1 text-2xl font-black text-slate-950 md:text-3xl">{t('Danh sách ứng viên')}</h1>
        <p className="mt-2 text-sm text-slate-500">{t('Quản lý học viên tiềm năng, thông tin phụ huynh, chứng chỉ, khóa học mong muốn và lịch rảnh.')}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button onClick={onToggleForm}><Plus size={18} /> {t('Thêm mới ứng viên')}</Button>
